...
public final class MathUtils {
	...
	/**
	 * Calculate the actual data interval.
	 * @param intervals
	 * @return
	 */
	public static long[][] mergeInterval(long[][] intervals) {
		//sort to get ordered array
		Arrays.sort(intervals, (o1, o2) -> (int) (o1[0] - o2[0]));
		List<long[]> path = new ArrayList<>();
		int left = 0, right = 0;
		while (right < intervals.length) {
			long leftBound = intervals[left][0];
			long rightBound = intervals[right][1];
			...
			path.add(new long[]{leftBound, rightBound});
			left = right;
		}
		//copy the result to the array from List
		long[][] res = new long[path.size()][2];
